...
class AnswerButton:
    ...
    def __init__(self, answer, index, frame, callback):
        self._frame = frame
        self._index = index
        self._answer_button = Button(frame, text=str(answer), command=lambda: callback(index), relief=GROOVE, padx=10, pady=10)
        self._answer_button.config(font=(font_type, 14), bg=green, border=1, fg=white)
...
class QuestionBox:
    ...
    def generate_single_answer_button(self, frame, answer, index, line_max_len, longest_answer):
        ...
        answer_button.config(height=longest_answer / line_max_len, width=max(longest_answer, line_max_len))
        return answer_button

    def find_longest_answer_len(self):
        longest_answer = 0
        for answer in self._question.get_answers_list():
            longest_answer = max(len(str(answer)), longest_answer)
        return longest_answer

    def generate_question(self):
        label = Label(self._master, text=str(self._question), wraplength=800)
    ...
    def generate_correct_answer_label(self):
        label = Label(self._master, text=correct_answer_str + str(self._question.get_correct_answer()))
    ...
    def generate_close_button(self):
        close_button = Button(self._master, text=close_button_str, command=self._master.destroy, font=(font_type, 14), relief=GROOVE, bd=1, background=red, fg=white)
        return close_button
